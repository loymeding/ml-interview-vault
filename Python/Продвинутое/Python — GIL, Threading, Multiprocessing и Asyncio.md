...
> - I/O-bound — большая часть времени уходит на ожидание сети, диска или базы.
> - CPU-bound — большая часть времени уходит на вычисления.
> - ThreadPoolExecutor удобен для блокирующих I/O-функций.
> - asyncio удобен для большого количества неблокирующих I/O-операций.
> - ProcessPoolExecutor нужен для тяжёлого чистого Python-кода, но требует сериализации аргументов и результатов.
> - async def не делает синхронную функцию неблокирующей. Блокирующий вызов нужно заменить на async-клиент или вынести в поток.
...
### 4.1. Task, Future, gather и отмена

Корутина — описание асинхронной работы. Task — запланированная к выполнению корутина. Future — объект, который обещает будущий результат.
...
Параметр return_exceptions позволяет получить ошибки как элементы результата и отдельно решить, можно ли продолжать. Без него первое исключение обычно прерывает gather.

При отмене запроса нужно корректно закрывать соединения и освобождать ресурсы. Таймаут и отмена должны быть частью контракта функции, а не случайным исключением где-то глубоко в стеке.

### 4.2. Синхронный код внутри async-сервиса
...
to_thread помогает сохранить отзывчивость event loop, но не делает CPU-heavy функцию по-настоящему параллельной. Для тяжёлых вычислений нужен процесс или библиотека, работающая вне Python-байткода.
