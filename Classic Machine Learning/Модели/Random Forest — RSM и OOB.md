...
## Гиперпараметры и когда их тюнить

- `n_estimators` (число деревьев): Чем больше, тем лучше, но дольше учить. Обычно ставят 100–500, дальше прирост качества асимптотически затухает.
- `max_features` (размер случайного подпространства $m$): Ключевой гиперпараметр для балансировки корреляции и силы деревьев. Если много шумовых признаков, $m$ стоит уменьшить.
- `max_depth` (максимальная глубина): По умолчанию деревья строятся до конца (чтобы минимизировать bias). **Ограничивать глубину стоит, когда:**
  1. В данных очень много шума (ограничение сработает как регуляризация).
  2. Модель весит слишком много мегабайт или гигабайт (деревья растут экспоненциально).
  3. Требуется очень быстрый инференс.

## Вычислительная сложность (Время и Память)
- **Время инференса (предсказания):** $O(n_{\text{trees}} \cdot d)$, где $d$ — средняя глубина деревьев. Предсказание происходит очень быстро и легко распараллеливается, так как деревья независимы.

## Extremely Randomized Trees (Extra Trees)

Экстремальный вариант Random Forest. Отличия от обычного RF:
1. **Не использует Bootstrap.** Каждое дерево учится на всей выборке.
2. При выборе сплита перебираются не все возможные пороги для признака, а **выбирается случайный порог**.

**Эффект:** Деревья строятся гораздо быстрее, их корреляция падает еще сильнее, а variance ансамбля снижается. Extra Trees часто работает лучше на сильно зашумленных данных.
