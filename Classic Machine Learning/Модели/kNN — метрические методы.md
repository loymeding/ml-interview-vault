...
$$\Large d_p(x, y) = \left(\sum_{i=1}^n |x_i - y_i|^p\right)^{1/p}$$

где:
- $d_p$ — расстояние Минковского с параметром $p$
- $x, y$ — два объекта (векторы признаков)
- $x_i, y_i$ — значения $i$-го признака объектов $x$ и $y$
- $n$ — количество признаков (размерность пространства)
- $p$ — параметр метрики: 1 = Манхэттен, 2 = Евклид, $\infty$ = Чебышёв
...
$$\Large \hat{y} = \frac{1}{k} \sum_{i \in N_k(x)} y_i$$

где:
- $\hat{y}$ — предсказание для нового объекта $x$
- $k$ — количество ближайших соседей
- $N_k(x)$ — множество индексов $k$ ближайших соседей объекта $x$
- $y_i$ — таргет $i$-го соседа из обучающей выборки
...
$$\Large \hat{y}(x) = \frac{\sum_{i=1}^n K\left(\frac{d(x, x_i)}{h}\right) \cdot y_i}{\sum_{i=1}^n K\left(\frac{d(x, x_i)}{h}\right)}$$

где:
- $\hat{y}(x)$ — взвешенное предсказание для нового объекта $x$
- $K(\cdot)$ — функция ядра (Гауссово, Епанечникова и др.) — задаёт форму спада веса с расстоянием
- $d(x, x_i)$ — расстояние от $x$ до $i$-го обучающего объекта
- $h$ — bandwidth (ширина окна): чем меньше $h$, тем резче убывает вес с расстоянием
- $y_i$ — таргет $i$-го обучающего объекта
...
$$\Large d = \sqrt{(1\,000\,000)^2 + (5)^2} \approx 1\,000\,000$$
...
- $\mu$ — среднее признака по обучающей выборке
...
$$\Large \hat{y}(x) = \frac{\sum_{i \in N_k} w_i \cdot y_i}{\sum_{i \in N_k} w_i}, \quad w_i = \frac{1}{d(x, x_i)}$$

где:
- $\hat{y}(x)$ — взвешенное предсказание для объекта $x$
- $N_k$ — множество $k$ ближайших соседей
- $w_i$ — вес $i$-го соседа: обратно пропорционален расстоянию
- $y_i$ — таргет $i$-го соседа
- $d(x, x_i)$ — расстояние от $x$ до $i$-го соседа
