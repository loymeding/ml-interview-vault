...
Если деревья **некоррелированы**, дисперсия среднего:

$$\text{Var}\left(\frac{1}{T}\sum_t \hat{y}_t\right) = \frac{\sigma^2}{T}$$

В $T$ раз меньше дисперсии одного дерева. Каждое дерево глубокое (низкий bias), а усреднение убирает их «шум» (variance).

Некоррелированность достигается через:
- Случайные подвыборки объектов (bootstrap)
- Случайный отбор признаков на каждом сплите (`max_features`)
