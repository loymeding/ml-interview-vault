...
|---|---|---|
| **Порядок** | Параллельно | Последовательно |
| **Что снижает** | Variance (Дисперсию) | Bias (Смещение) |
| **Базовые модели** | Глубокие деревья | Неглубокие деревья |
| **Переобучение** | Почти не боится (LLN) | Критически боится |
| **Чувствительность к шуму** | Низкая | Высокая |
| **Инференс** | Параллельный (быстрый) | Последовательный (медленнее) |

## Короткий пример

**Консилиум (Bagging) vs Обучение (Boosting):**
- **Bagging:** У вас есть 100 студентов. Каждый прочитал свою главу учебника (bootstrap). Вы спрашиваете всех и берете средний ответ. Если один студент пропустил лекцию, остальные 99 вытянут результат. **Сила в разнообразии.**
- **Boosting:** У вас есть один студент. Он читает учебник, делает ошибки. Преподаватель указывает на ошибки. Студент читает снова, фокусируясь на трудных местах. И так 100 раз. В конце студент знает предмет идеально. **Сила в последовательном исправлении.**

## Bias-Variance Tradeoff

- **Bagging** не меняет Bias. Поэтому нам нужны базовые модели с **низким смещением** (глубокие деревья). Их высокая дисперсия — "проблема", которую решает усреднение.
- **Boosting** уменьшает Bias на каждом шаге. Поэтому базовые модели могут быть **слабыми** (высокое смещение, зато низкая дисперсия).
