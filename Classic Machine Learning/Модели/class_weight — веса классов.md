...
> Oversampling меняет **распределение данных** → вероятности модели сдвигаются, нужна пост-калибровка. class_weight меняет **веса в оптимизации** → исходное распределение сохраняется, предсказанные вероятности остаются калиброванными.

## Когда class_weight может ухудшить качество?

- **Дисбаланс информативен:** если реальная база редкого события мала (например, 1% дефолтов — это правда мало), то большой вес на редкий класс может заставить модель «видеть» дефолты везде
- **Шум в редком классе:** если редкие объекты некачественно размечены, их высокий вес усиливает шум
- **Задача требует откалиброванных вероятностей:** сильные веса могут нарушать калиброванность
